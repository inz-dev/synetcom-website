...
const createNewRecord=()=>useForm ({
  ...
  const isEditing = toRef(() => !!formModel.value.id_departement)
  ...
  onMounted(() => {
    //console.log("props.allDepartments:",props.allDepartments,departements)
...
if(found || found.length!=0){
  ...
  function save () {
    ...
    if (isEditing.value) {
      const index = departements.findIndex(dept =>dept.id_departement=== formModel.value.id_departement)
      console.log('index from save:', index);

      departements[index] = formModel.value
      console.log(departements[index] );

    } else {
      formModel.value.id_departement = departements.value.length + 1
      departements.push(formModel.value)
    }
  ...
  function reset () {
    dialog.value = false
    formModel.value = createNewRecord()
    departements.value = [props.allDepartments]
  }
</script>
